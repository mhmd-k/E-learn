...
async function getQuestions() {
  const res = await fetch(`../assessments/${course.name}.json`);
...
function finish() {
  ...
  if (score >= 60) {
    endGameSection.querySelector("h2").innerHTML = `
        You have passed <br /> Your Score is: ${score} out of 100`;
    endGameSection
      .querySelector("img")
      .setAttribute(
        "src",
    ...
    endGameSection.querySelector("h2").innerHTML = `
        YOU didn't pass <br /> Your Score is: ${score} out of 100 <br /> Score 60 or higher to pass`;
    endGameSection
      .querySelector("img")
